...
export async function getChapters(preview = false) {
  return getEntries<ChapterFields>('chapter', { order: ['fields.name'] }, preview);
}
...
export async function getProjects(preview = false) {
  return getEntries<ProjectFields>('project', { order: ['-fields.date'] }, preview);
}
...
export async function getPartners(preview = false) {
  return getEntries<PartnerFields>('partner', { order: ['fields.name'] }, preview);
}
...
export async function getJournalPosts(preview = false) {
  return getEntries<JournalPostFields>('journalPost', { order: ['-fields.publishDate'] }, preview);
}
